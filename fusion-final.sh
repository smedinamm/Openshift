...
podman login registry.redhat.io --authfile $HOME/pull-secret.json
...
opm render registry.redhat.io/redhat/redhat-operator-index:v4.12 | jq 'select( .package == "amq-streams" or .name == "amq-streams")' > pruned-catalog/configs/index.json
opm generate dockerfile pruned-catalog/configs; cd pruned-catalog/
podman build -t $TARGET_PATH/data-cataloging-redhat-operator-index:v4.12 -f configs.Dockerfile .
podman push $TARGET_PATH/data-cataloging-redhat-operator-index:v4.12
nohup oc adm catalog mirror -a "$LOCAL_SECRET_JSON" "$TARGET_PATH"/data-cataloging-redhat-operator-index:v4.12 "$TARGET_PATH" --index-filter-by-os='linux/amd64' &
...
spec:
  ...
  image: "$TARGET_PATH/data-cataloging-redhat-operator-index:v4.12"
  sourceType: grpc
EOF
